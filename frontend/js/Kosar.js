...
export class Kosar {
  ...
  hozzaad(termekAdat) {
    const letezoElem = this.#lista.find(
      (elem) => elem.getTermekId() === termekAdat.id,
    );
  ...
  megjelenit() {
    ...
    if (this.#lista.length === 0) {
      ...
      this.#lista.forEach((elem) => elem.megjelenit());
      ...
      this.#szuloElem.insertAdjacentHTML(
        ...
        `
            <div class="osszesito">
                ...
                <button id="rendeles-btn" class="buy-button">Megrendelem</button>
            </div>
        `,
      );
